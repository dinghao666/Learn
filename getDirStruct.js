...
// 文件名在下面数组中项的都排除掉
const excludeFile = ['img', 'LICENSE', 'README.md', path.basename(__filename)]  //path.basename(__filename) 表示当前执行文件
...
// 获取所有文件路径
;(function getDirStruct(basePath = __dirname) {  //basePath = __dirname表示当前文件所在的目录
  const files = fs.readdirSync(basePath)    //返回当前目录下所有文件名称的数组对象
  // 空文件夹处理
  if (files.length === 0) {
